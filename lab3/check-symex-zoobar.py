...
def test_stuff():
  ...
  usercount_before = len(pdb.query(zoobar.zoodb.Person).all())
  
  balance_map = {}
  for p in pdb.query(zoobar.zoodb.Person).all():
    balance_map[p.username] = p.zoobars
  ...
  usercount_after = len(pdb.query(zoobar.zoodb.Person).all())
  if usercount_before == usercount_after and balance_before != balance_after:
    report_balance_mismatch()

  ## Detect zoobar theft.
  ## When detected, call report_zoobar_theft()
  for p in pdb.query(zoobar.zoodb.Person).all():
    log = tdb.query(zoobar.zoodb.Transfer).filter(zoobar.zoodb.Transfer.sender == p.username).all()
    if len(log) == 0 and p.zoobars < balance_map[p.username]:
        report_zoobar_theft()
