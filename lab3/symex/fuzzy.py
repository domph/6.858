...
class concolic_int(int):
  ...
  def __mul__(self, o):
    res = self.__v * (o.__v if isinstance(o, concolic_int) else o)
...
class concolic_str(str):
  ...
  def __contains__(self, o):
    return concolic_bool(sym_contains(ast(self), ast(o)), (o.__v if isinstance(o, concolic_str) else o) in self.__v)
...
class concolic_bytes(bytes):
  ...
  def __contains__(self, o):
    return concolic_bool(sym_contains(self, ast(o)), (o.__v if isinstance(o, concolic_bytes) else o) in self.__v)
...
def concolic_execs(func, maxiter = 100, verbose = 0):
  ...
  while iter < maxiter and not inputs.empty():
    ...
    for i in range(len(branch_conds)):
      new_constr = concolic_force_branch(i, branch_conds, branch_callers, verbose)
      if new_constr not in checked:
        checked.add(new_constr)
        
        # find possible input
        ok, new_cv = concolic_find_input(new_constr, concrete_values.var_names(), verbose)
        if ok:
          new_cv.inherit(concrete_values)
          inputs.add(new_cv, branch_callers[i])
